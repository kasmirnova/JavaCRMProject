...
This is a tiny (~1.09k brotli'd) library that mostly mirrors the IndexedDB API, but with small improvements that make a
...
import `idb/with-async-ittr.js` instead of `idb` (this increases the library size to ~1.3k brotli'd):
